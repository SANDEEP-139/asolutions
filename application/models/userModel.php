...
class userModel extends database{
    ...
    public function check_employee($sms_contact){
        if($this->query("select * from teachers where sms_contact=?",[$sms_contact])){
            return true;
        }else{return false;}
    }
    ...
    }public function select_teacher_data($data){
        if($this->query("select * from teachers where id=?",[$data])){
            return $this->fetch();
        }else{return false;}
    }
    ...
    public function search_transactions($query){
        if($this->query($query)){
            return $this->fetchall();
        }else{return false;}
    }
     public function update_students($data){
        if($this->query("update students SET fathers_name=?,mothers_name=?,date_of_birth=?,class=? where id=?",$data)){
            return true;
         }else{return false;}
        
      
     }
     public function update_mappings($data){
         if($this->query("update students SET lagititude=?,langititude=? where id=?",$data)){
             return true;
         }else{return false;}

     }
     public function photo_updates($data){
         if($this->query("update students SET student_photo=? where id=?",$data)){
             return true;
         }else{return false;}
     }
     public function contact_updates($data){
        if($this->query("update students SET father_contact1=?,sms_contact_no=? where id=?",$data)){
            return true;
        }else{return false;}
    } 
    public function roll_update($data){
        if($this->query("update students SET rollo_number=? where id=?",$data)){
            return true;
        }else{return false;}

    }
    public function delete_employee_record($data){
        if($this->query("delete from teachers where id=?",$data)){
            return true;
        }else{return false;}
    }
    public function employee_updates($data){
        if($this->query("update teachers SET employee_name=?,father_name=? where id=?",$data)){
            return true;
        }else{return false;}
    } 
